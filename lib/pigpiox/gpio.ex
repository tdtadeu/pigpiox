defmodule Pigpiox.GPIO do
  ...
  @gpio_pud Map.keys(@gpio_pud_map)
  ...
  @typedoc """
  GPIO pull-up/pull-down resistor settings, for `set_pull_up_down/2`
  """
  @type pud :: :up | :down | :off
  ...
  @doc """
  Sets or clears the internal GPIO pull-up/down resistor for a specific GPIO `pin`.
  `pin` must be a valid GPIO pin number for the device, with some exceptions.
  See pigpio's [documentation](http://abyz.co.uk/rpi/pigpio/index.html) for more details.
  `pud` can be any of `t:pull/0`.
  """
  @spec set_pull_up_down(pin :: integer, pud) :: :ok | {:error, atom}
  def set_pull_up_down(pin, pud) when pud in @gpio_pud do
    case Pigpiox.Socket.command(:set_pull_up_down, pin, @gpio_pud_map[pud]) do
      {:ok, _} -> :ok
      error -> error
    end
  end
